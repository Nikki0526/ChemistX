...
def load_data(p_train, p_train_add):
    ...
    data_wetlab = pd.read_csv(p_train_add)  # This is from data_test folder
...
def model_fttransformer_wetlab(p_train, p_train_add, fnum = 32):
    ...
    principalComponents = pca.fit_transform(X)
    data_pca = pd.DataFrame(principalComponents)
    #data_pca = pd.DataFrame(feature) # Use PCA to reduce dimensions to accelerate training
    data_pca['label'] = data_projected_vector['yield_number'].tolist()
    ...
    test_dataset = df_to_dataset(test_data[FEATURES + [LABEL]], LABEL, shuffle=False) # No target, no shuffle Make sure the label is included when converting to dataset
...
def train_and_search(p_train, p_train_add, p_test, p_test_add, p_en, t_model, f_nums):
    ...
    # Collect all SMILES strings into a single list
    data = raw[names]
    smiles_all = []
    ...
    for i in range(len(data_output)):
        wd.find_element('id', 'smiles').clear()  # Find the input box and clear it
        element = wd.find_element('id', 'smiles')
        element.send_keys(data_output.at[i, 'smiles'] + '\n')  # Enter the string into the input box
        time.sleep(2)
        label = wd.find_elements('xpath', "//p")  # Get the output
    ...
    cimg1 = DataFrame(c)  # Convert dictionary into a DataFrame
    ...
    # Fill in missing additive CIMG vectors (use average since Suzuki reaction dataset lacks them)
